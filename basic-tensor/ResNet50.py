...
def create_var(name, shape, initializer, trainable=True):
    return tf.get_variable(name + str(random.random()), shape=shape, dtype=tf.float32,
...
def batch_norm(x, decay=0.999, epsilon=1e-03, is_training=True,
    ...
    with tf.variable_scope(scope + str(random.random())):
...
def _block(x, n_out, n, init_stride=2, is_training=True, scope="block"):
    with tf.variable_scope(scope):
        h_out = n_out // 4
        out = _bottleneck(x, h_out, n_out, stride=init_stride, is_training=is_training, scope="bottlencek1")

        for i in range(1, n):
            out = _bottleneck(out, h_out, n_out, is_training=is_training,
                          scope=("bottlencek%s" % (i + 1)))
        return out
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        with tf.variable_scope('block1'):
            net = conv2d(inputs, 64, 7, 2, scope="conv")  # -> [batch, 112, 112, 64]
            net = tf.nn.relu(batch_norm(net, is_training=is_training, scope="bn1"))
            # net = max_pool(net, 3, 2, scope="maxpool1")  # -> [batch, 56, 56, 64]
            net = tf.nn.max_pool(net, [1, 3, 2, 1], [1, 2, 2, 1], padding="SAME")

        net = _block(net, 256, 3, init_stride=1, is_training=is_training,
                     scope="block2")  # -> [batch, 56, 56, 256]

        # net = _block(net, 512, 4, is_training=is_training, scope="block3")
        # # -> [batch, 28, 28, 512]
        # net = _block(net, 1024, 6, is_training=is_training, scope="block4")
        # # -> [batch, 14, 14, 1024]
        # net = _block(net, 2048, 3, is_training=is_training, scope="block5")
        # # -> [batch, 7, 7, 2048]
        # net = avg_pool(net, 7, scope="avgpool5")  # -> [batch, 1, 1, 2048]
        # net = tf.squeeze(net, [1, 2], name="SpatialSqueeze")  # -> [batch, 2048]
        # logits = fc(net, num_classes, "fc6")  # -> [batch, num_classes]
        ...
        _ = tf.train.import_meta_graph("./model.pb")
